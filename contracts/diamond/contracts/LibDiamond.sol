...
library LibDiamond {
    ...
    function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        if (_facetAddress == address(0)) {
            revert AddFacetCantBeAddressZero();
        }
    ...
    function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        DiamondStorage storage ds = diamondStorage();
        if (_facetAddress == address(0)) {
            revert ReplaceFacetCantBeAddressZero();
        }
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            ...
            if (oldFacetAddress == _facetAddress) {
                revert ReplaceFunctionWithSameFunction();
            }
    ...
    function removeFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        DiamondStorage storage ds = diamondStorage();
        if (_facetAddress == address(0)) {
            revert RemoveFacetAddressMustBeAddressZero();
        }
    ...
    function removeFunction(DiamondStorage storage ds, address _facetAddress, bytes4 _selector) internal {
        if (_facetAddress == address(0)) {
            revert CantRemoveFunctionThatDoesntExist();
        }
        if (_facetAddress == address(this)) {
            revert CantRemoveImmutableFunction();
        }
